...
function applyAndRender(rows, headers, source){
  ...
  const rankMap = {};
  data.forEach((row, i) => { const key = String(row['No Member'] ?? '').trim(); if (key) rankMap[key] = i + 1; });
  state.rows = data; state.rankMap = rankMap;
  renderPodium(state.rows); updateStats(); applyFilter(); render();
  el.meta.textContent = `Sumber: ${source} • ${state.filtered.length} baris • ${new Date().toLocaleString('id-ID')}`;
}

function applyFilter(){
  const q = state.query.trim().toLowerCase();
  state.filtered = !q ? [...state.rows] : state.rows.filter(r => String(r['No Member']).toLowerCase().includes(q) || String(r['Nama']).toLowerCase().includes(q));
}
...
function render(){
  ...
  if (isSearching){
    if (state.filtered.length === 0){ el.info.style.display='none'; el.info.innerHTML=''; }
    else {
      const r = state.filtered[0]; const no = String(r['No Member'] ?? '').trim(); const nama = String(r['Nama'] ?? '').trim(); const pts = Number(r['Point'] ?? 0); const rank = state.rankMap[no] ?? '-'; const gap = Math.max(top1Points - pts, 0);
      el.info.innerHTML = `
  ...
  } else { el.info.style.display='none'; el.info.innerHTML=''; }
  ...
  if (slice.length===0){ el.body.innerHTML = `<tr><td colspan="4" class="empty">Tidak ada data.</td></tr>`; return; }
